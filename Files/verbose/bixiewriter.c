...
static void prettyPrint(element_t* ast);

static void printElement(element_t* element, int depth);
...
void encodeASTToBIXIE(element_t* ast, byte** bixieBuffer, int* bufferLength, symbolTable_t symbolTable) {
  ...
  int allocated = BIXIE_BUFFER_SIZE; // Needs removal
  ...
  *bufferLength = 0;
  ...
  printf("Done with pretty printing\n");

  while (element != NULL) {
    printf("---Encoding element %s---\n", element->name);
	
    printf("Buffer length is %d and size is %d\n", *bufferLength, allocated);
...
void encodeElement(symbolTable_t* symbolTable, byte** bixieBuffer, int* bufferLength, int* allocated, element_t* element, element_t* leftSibling) {
  ...
  if (element->parent == NULL) {
    ...
    printf("Copying encoded element ID %d to buffer. Encoded Length is %d\n", element->ID, encodingLength);
    ...
    if (leftSibling == NULL || leftSibling->ID != element->ID) {
      ...
      printf("Index in parents sequence %d and leading 0's start at index %d\n", sequenceIndex, i);
      for ( ; i <= sequenceIndex; i++) {
        encodeCIntValue(element->parent->content.complex.childrenCount[i], &encodingBuffer, &encodingLength);
        printf("Copying encoded sequence count %d to buffer. Encoded Length is %d\n", element->parent->content.complex.childrenCount[i], encodingLength);
        copyToBixieBuffer(encodingBuffer, encodingLength, bixieBuffer, bufferLength, allocated);      
      }
    }
  }
...
static void copyToBixieBuffer(byte* encodingBuffer, int encodingLength, byte** bixieBuffer, int* bufferLength, int* allocated) {
  ...
  if (leftOfBuffer < encodingLength) {
    printf("Bixie buffer is too small: %d < %d! will be resized!\n", leftOfBuffer, encodingLength);
...
static void encodeTrailingSequence(element_t* element, int listSize, byte** bixieBuffer, int* bufferLength, int* allocated) {
  ...
  printf("Trailing 0's sequence start at index %d and end at %d\n", i, listSize-1);
  for ( ; i < listSize; i++) {
    byte* encodingBuffer;
    int encodingLength;
    encodeCIntValue(element->content.complex.childrenCount[i], &encodingBuffer, &encodingLength);
    printf("Copying encoded sequence count %d to buffer. Encoded Length is %d\n", element->content.complex.childrenCount[i], encodingLength);
    copyToBixieBuffer(encodingBuffer, encodingLength, bixieBuffer, bufferLength, allocated);      
  }
}
...
static void encodeAttributes(symbolTable_t* symbolTable, byte** bixieBuffer, int* bufferLength, int* allocated, element_t* element, elementTable_t* elementTable) {
  ...
  printf("Copying encoded attribute count %d to buffer. Encoded Length is %d\n", element->attributeCount, encodingLength);
  copyToBixieBuffer(encodingBuffer, encodingLength, bixieBuffer, bufferLength, allocated);
  ...
  for (i = 0; i < element->attributeCount; i++) {
    ...
    printf("Copying encoded attribute ID %d to buffer. Encoded Length is %d\n", attributeID, encodingLength);
    copyToBixieBuffer(encodingBuffer, encodingLength, bixieBuffer, bufferLength, allocated);
    ...
    printf("Copying encoded attribute simple type \"%s\" simple ID %d to buffer. Encoded Length is %d\n", element->attributes[i].value, simpleTypeID, encodingLength);
    copyToBixieBuffer(encodingBuffer, encodingLength, bixieBuffer, bufferLength, allocated);
  }
}
...
static void encodeContent(symbolTable_t* symbolTable, byte** bixieBuffer, int* bufferLength, int* allocated, element_t* element, elementTable_t* elementTable) {
  ...
  if (element->type == choice) {
    ...
    printf("Copying encoded choice count %d to buffer. Encoded Length is %d\n", element->content.complex.childrenCount[0], encodingLength);
    copyToBixieBuffer(encodingBuffer, encodingLength, bixieBuffer, bufferLength, allocated);
    ...
    printf("Copying encoded choice ID %d to buffer. Encoded Length is %d\n", childID, encodingLength);
    copyToBixieBuffer(encodingBuffer, encodingLength, bixieBuffer, bufferLength, allocated);
  }
    ...
    printf("Copying encoded element simple type \"%s\" simple ID %d to buffer. Encoded Length is %d\n", element->content.simple.value, simpleTypeID, encodingLength);
    copyToBixieBuffer(encodingBuffer, encodingLength, bixieBuffer, bufferLength, allocated);
  }
    ...
    for (i = 0; i < listSize; i++) {
      ...
      printf("Copying encoded empty sequence index %d to buffer. Encoded Length is %d\n", i, encodingLength);
      copyToBixieBuffer(encodingBuffer, encodingLength, bixieBuffer, bufferLength, allocated);      
    }
  }
}
...
/**
 * Traverses the AST in a left child->right sibling->parent loop. At each element it prints the values of the element
 *
 * @param ast           The AST to encode into BIXIE
 *
 * @return
...
static void prettyPrint(element_t* ast) {
  ...
  printf("Pretty print traversal of AST\n\n");
...
/**
 * Prints the values of a given element and indents the text based on a depth
 *
 * @param element       The element to be printed
 * @param depth         The current depth of the print
 *
 * @return
 */
static void printElement(element_t* element, int depth) {
  ...
  //Print indent spaces and element name
  for (i = 0; i < depth; i++)
    printf("  ");
  printf("ELEMENT %s ID %d\n", element->name, element->ID);

  //Print indent spaces and attributes
  for (i = 0; i < element->attributeCount; i++) {
    int j;
    for (j = 0; j < depth+1; j++)
      printf("  ");
    printf("ATTRIBUTE %d %s=%s\n", element->attributes[i].ID, element->attributes[i].name, element->attributes[i].value);
  }

  // Print indent spaces and content
  for (i = 0; i < depth+1; i++)
    printf("  ");
  switch (element->type) {
    case simple :
      printf("CONTENT=simple\n"); break;
    case sequence :
      printf("CONTENT=sequence\n"); break;
    case choice :
      printf("CONTENT=choice\n"); break;
    case empty :
      printf("CONTENT=empty\n"); break;
  }

  // Print indent spaces and simpleType contents
  if (element->type == simple) {
    int j;
    for (j = 0; j < depth+1; j++)
      printf("  ");
    printf("LENGTH=%d SIMPLETYPE=%s\n", strlen(element->content.simple.value), element->content.simple.value);
  }

  // Print indent spaces and complex contents
  if (element->type == sequence || element->type == choice) {
    int i;
    for (i = 0; i < element->content.complex.childrenListSize; i++) {
      int j;
      for (j = 0; j < depth+1; j++)
        printf("  ");
      printf("ELEMENTCOUNT=%d\n", element->content.complex.childrenCount[i]);
      if (i > 10) {
        break;
      }
    }
...
static freeASTElement(element_t* element) {
  if (element == NULL) {
    printf("Trying to free a already free'd element\n");
    return;
  }
  free(element->name);
